...
/**
 * Created by Lance on 2019/3/26.
 * QQ:2654074904
 * description:彩色柱状图 可从下方断开显示
 */
public class HistogramView extends View {

    //字符串时间(12:00:00)转化工具类
    ...
    private Rect mBottomTextBounds;//测量文字宽高使用
    ...
    private Paint verticalLinePaint;//柱状图画笔
    private int VerticalLineWidth = 40;//柱状图宽度

    private Paint drawLinePaint;//横纵轴参考线
    ...
    public HistogramView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        verticalLinePaint.setColor(Color.parseColor("#55B0DD"));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //横 坐标轴参考线
        canvas.drawLine(startX, startY, getWidth() - rightReservedWidth, startY, drawLinePaint);
        //纵 坐标轴参考线
        canvas.drawLine(startX, startY, startX, topReservedHeight, drawLinePaint);
        ...
        for (int i = 0; i < 7; i++) {
            ...
            canvas.drawText(timeTextList.get(i), 0, timeTextList.get(i).length(), timeTextMarginLeft,
                    ...
                    mDateTimeTextPaint);
            //横向日期
            canvas.drawText(requestResult.getBodyStatusList().get(i).getDayOfWeek(), 0, requestResult.getBodyStatusList().get(i).getDayOfWeek().length(),
...
//                    startY, startX + ((getWidth() - rightReservedWidth - startX) / 8) * (i + 1), topReservedHeight, verticalLinePaint);
//

            List<WeekSleepRangesBean> sleepRanges = bodyStatusList.get(i).getSleepRanges();

            WeekSleepRangesBean preRange = sleepRanges.get(0);
            ...
            for (int j = 0; j < sleepRanges.size(); j++) {
                ...
                boolean isContinue = drawVerticalLine(rangesBean, canvas, i);
                if (isContinue) {
                    continue;
                }
                ...
                try {
                    ...
                    DateTime preEndTime = new DateTime(dateTimeFormatter.parse(preRange.getEndTime()));
                    DateTime currStartTime = new DateTime(dateTimeFormatter.parse(rangesBean.getStartTime()));
                    if (!preEndTime.isEqual(currStartTime) && preEndTime.isBefore(currStartTime)) {
                        WeekSleepRangesBean continuousBean = new WeekSleepRangesBean();
                        continuousBean.setStartTime(preRange.getEndTime());
                        continuousBean.setEndTime(rangesBean.getStartTime());
                        continuousBean.setSleepStatus(-1);//超过6为灰色
                        drawVerticalLine(continuousBean, canvas, i);
                    }
                    preRange = rangesBean;
                } catch (ParseException e) {
                    e.printStackTrace();
...
//                columnEndY = zeroOrigin12Y - (preHouseHeight * (endHouse + zeroOrigin12House)) - (preMinuteHeight * startMinute);
//            }
//            verticalLinePaint.setColor(getExplainColor(rangesBean.getSleepStatus()));
//            canvas.drawLine(startX + ((getWidth() - rightReservedWidth - startX) / 8) * (i + 1),
//                    columnStartY, startX + ((getWidth() - rightReservedWidth - startX) / 8) * (i + 1), columnEndY, verticalLinePaint);
            //<------
            //纵向横线
//            canvas.drawLine(startX, topReservedHeight + (startY - topReservedHeight) / 6 * i, getWidth() - rightReservedWidth, topReservedHeight + (startY - topReservedHeight) / 6 * i, drawLinePaint);
            canvas.drawLine(startX, topReservedHeight + (startY - topReservedHeight) / 6 * i, getWidth() - rightReservedWidth, topReservedHeight + (startY - topReservedHeight) / 6 * i, drawLinePaint);
        }
        ...
        //测试
//        canvas.drawLine(timeTextMarginLeft, getHeight() - getBottomExplainHeight(), getWidth(), getHeight() - getBottomExplainHeight(), drawLinePaint);
    }

    //计算坐标
    public boolean drawVerticalLine(WeekSleepRangesBean rangesBean, Canvas canvas, int i) {
